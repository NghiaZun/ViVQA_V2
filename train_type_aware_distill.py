...
class TrainConfig:
    ...
    batch_size: int = 2                 # REDUCED from 4 to 2 (OOM fix)
    accum_steps: int = 16               # INCREASED from 8 to 16 (keep effective batch = 32)
    num_epochs: int = 20
    val_ratio: float = 0.1
    num_workers: int = 2                # REDUCED from 4 to 2 (less memory)
...
def get_loss_weights(stage: int, temperature: float = 2.0):
    """Get loss weights based on training stage - GT-GUIDED (no teacher embeddings)
    ...
    - NO embedding alignment (teacher model not loaded to save memory)
    ...
    Returns:
        Dictionary with loss weights (NO 'gt', 'vision', 'text' - not used!)
    """
    if stage == 1:
...
def train():
    ...
    # NO TEACHER MODEL LOADING - GT-guided training uses teacher LABELS only (no embeddings)
    print("[INFO] ⚠️  Skipping teacher model loading (GT-guided uses labels only)")
    print("  - Teacher embeddings NOT used (vision/text alignment disabled)")
    print("  - Loss composition: Teacher labels (0.7-0.8) + Type (0.1)")
    print("  - Memory saved: ~7-8GB from not loading Qwen-7B")
    teacher = None
    ...
    print(f"  🧠 Reasoning: Generated by Qwen-7B (offline)")
    print(f"  📊 NO separate GT loss (redundant - teacher already contains GT!)")
    print(f"  🚫 NO teacher embeddings (teacher model NOT loaded to save memory)")
    print(f"\n[LOSS COMPOSITION]")
    print(f"  🎯 Teacher loss (0.9): Learn answer + reasoning together")
    print(f"  🏷️  Type classification (0.1): Structured reasoning awareness")
    print(f"  ❌ Vision/Text alignment: DISABLED (no teacher model)")
    print(f"\n[TRAINING STRATEGY]")
    print(f"  Stage 1 (Epochs 1-{cfg.stage1_epochs}): Learn reasoning patterns")
    print(f"  Stage 2 (Epochs {cfg.stage1_epochs+1}-{cfg.stage1_epochs+cfg.stage2_epochs}): Master reasoning")
    print(f"  Stage 3 (Epochs {cfg.stage1_epochs+cfg.stage2_epochs+1}-{cfg.num_epochs}): Polish")
    print(f"\n[MEMORY OPTIMIZATION]")
    print(f"  - Batch size: {cfg.batch_size} (reduced from 4)")
    print(f"  - Gradient accumulation: {cfg.accum_steps} steps (effective batch = {cfg.batch_size * cfg.accum_steps})")
    print(f"  - Teacher model: NOT LOADED (~7GB saved)")
    print(f"{'='*70}\n")
